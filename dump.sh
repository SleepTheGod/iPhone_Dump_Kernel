...
install_packages() {
    ...
    apt install -y build-essential git autoconf automake libtool pkg-config \
                   libglib2.0-dev libusb-1.0-0-dev || { echo "Package installation failed!"; exit 1; }
    echo "All necessary packages installed."
}

# Function to compile libimobiledevice
compile_libimobiledevice() {
    echo "Cloning libimobiledevice repository..."
    git clone https://github.com/libimobiledevice/libimobiledevice.git || { echo "Failed to clone libimobiledevice"; exit 1; }

    cd libimobiledevice || { echo "Failed to change directory to libimobiledevice"; exit 1; }
    
    echo "Compiling libimobiledevice..."
    ./autogen.sh || { echo "Failed to run autogen.sh"; exit 1; }
    make || { echo "Failed to compile libimobiledevice"; exit 1; }
    make install || { echo "Failed to install libimobiledevice"; exit 1; }
    cd .. || { echo "Failed to return to previous directory"; exit 1; }
    
    echo "libimobiledevice compiled and installed successfully."
}
